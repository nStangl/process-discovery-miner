...
import { DropzoneAreaBase, DropzoneAreaBaseClasses, FileObject } from "material-ui-dropzone";
import AttachFile from '@material-ui/icons/AttachFile';
import Graph from './Graph';
import { ConstructionOutlined, RepeatOneSharp, UploadFile } from '@mui/icons-material';
        ...
        if (this.state.files === undefined || this.state.files.length < 1) {
            return (
                <div style={{ width: '100%', display: 'flex', justifyContent: 'center'}}>
                  <Box sx = {{ display: 'flex', justifyContent: 'center', p: 1, m: 1, width: '60%' }}>
                    <DropzoneAreaBase
            ...
            try {
                if (typeof this.state.files[0].data  === "string") {
                    if (this.state.files[0].data.match(MinerClass.regxRemoveMD)) {
                        dataString = this.state.files[0].data.replace(MinerClass.regxRemoveMD, "");
                    }
                    dataString = this.b64_to_utf8(dataString);
                }
            } catch (e) {
                console.log("An error has occured while trying to decode the file. Check if the content of your file has UTF-8 encoding!")
                dataString = "";
            }
            ...
            return (
                <GraphClass postBody = {dataString} miner = {"alphaminer"} />
            );
        }
